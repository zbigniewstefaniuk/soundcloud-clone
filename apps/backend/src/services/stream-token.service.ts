...
class StreamTokenService {
  private get streamSecret(): string {
    ...
    return createHmac("sha256", env.JWT_SECRET).update("stream-token-v1").digest("hex")
  }
  ...
  verifyStreamToken(token: string): StreamToken | null {
    try {
      ...
      // Decode payload
      const payload = JSON.parse(Buffer.from(data, "base64url").toString("utf8")) as StreamToken
  ...
  private sign(data: string): string {
    return createHmac("sha256", this.streamSecret).update(data).digest("base64url")
  }
