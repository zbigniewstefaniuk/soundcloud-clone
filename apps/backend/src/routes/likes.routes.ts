...
export const likeRoutes = new Elysia()
  ...
  .get(
    '/tracks/:id/likes',
    async ({ params, query }) => {
      const result = await likeService.getTrackLikes(params.id, query.page, query.pageSize)
  ...
  .get(
    '/users/me/likes',
    async ({ user, query }) => {
      const result = await likeService.getUserLikes(user.userId, query.page, query.pageSize)
      return paginated(result.data, result.pagination)
    },
    {
      query: PaginationSchema,
